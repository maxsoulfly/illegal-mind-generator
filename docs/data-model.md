...
**Storage:** the `saved_entries` table in Postgres (per project via `project_id`), reached through the local API (`server/routes/savedEntries.js`). Not localStorage.

**Hook:** `src/hooks/useSavedEntries.js`

### Identity — `id` is an immutable UUID

Since the 2026-09-08 identity refactor (migrations `0009`–`0012`):

- **`id`** is an immutable UUID (`saved_entries.id uuid PRIMARY KEY`, default `gen_random_uuid()`). A brand-new entry gets **one** UUID, assigned once (`crypto.randomUUID()` client-side, with the DB default as a fallback), and it never changes.
- **`artist`, `song`, `signalNumber` are editable data, not identity.** Editing Artist or Song and saving updates the **same row** (a rename), because Save `PUT`s the entry's existing UUID.
- **`matchKey`** (`saved_entries.match_key text NOT NULL`, non-unique index on `(project_id, match_key)`) is the normalized `` `${artist}-${song}`.trim().toLowerCase().replace(/\s+/g, ' ') `` string — recomputed on every write. It is used **only** for duplicate detection and bulk/import matching; it is **never** an id.
  - Saving a brand-new form whose `matchKey` matches an existing entry pops a `ConfirmDialog` ("Entry already exists / Update?") — Confirm adopts that entry's UUID, Cancel saves nothing. A rename of an already-loaded entry never triggers this.
  - `bulk` / `import` resolve an existing row by `matchKey` and reuse its UUID, else insert a fresh UUID row (non-interactive; import merge stays non-destructive).

`src/utils/savedEntries.js` / `server/savedEntryMerge.js` export `buildEntryMatchKey(artist, song)` for this — the two copies must stay byte-identical.

**Deleting an entry:** succeeds even when an Upload Calendar slot references it. The `upload_calendar_slots` FKs are `ON DELETE SET NULL` (migration `0012`), so the slot **row stays** and its `planned_entry_id` / `uploaded_entry_id` becomes `NULL` — there is **no** cascade delete of Calendar rows. `DELETE /saved-entries/:id` returns `{ projectId, id, deleted: true }` on success, `500 { error }` on a DB error.

### Fields

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `id` | `string` (UUID) | yes | Immutable identity, assigned once at creation. Not shown in the UI. |
| `matchKey` | `string` | yes | Normalized `artist-song` — duplicate/import matching only, never identity. Recomputed on every write. |
| `artist` | `string` | yes | Artist name, trimmed. Editable — a change renames the same row. |
| `song` | `string` | yes | Song name, trimmed. Editable — a change renames the same row. |
| `signalNumber` | `string` | no | Illegal Mind only — signal number (e.g. `"042"`). Editable data. |
...
- Entries are scoped to one project via `project_id`
- The Shorts Queue stores saved-entry **id references** (UUIDs), not snapshots (see [Shorts Queue](#shorts-queue))
- An Upload Calendar slot references a saved entry by UUID (`planned_entry_id` / `uploaded_entry_id`, FK `ON DELETE SET NULL`)
...
**Storage:** still localStorage (`shortsQueues[projectId].queue`) — deliberately **not** migrated to Postgres (a regenerable working list, not worth syncing between machines).
...
shortsQueues: {
  [projectId]: {
    queue: [savedEntryId]   // array of saved-entry id STRINGS, up to ~20
  }
}
```

The `queue` array holds saved-entry **id references, not snapshot copies**. Each render resolves an id to the live `savedEntries` entry, so later edits (tags, notes, rename) show up without re-randomizing; an id that no longer resolves renders as a skipped empty slot (never recreated).

**Id scheme (identity refactor):** new writes store the entry's **UUID** (`entry.id`). Legacy entries written before the UUID cutover hold the old normalized `artist-song` string; those are still resolved, as a **read-only fallback** against `entry.matchKey`. There is **no** automatic localStorage rewrite — a legacy id is replaced with a UUID only when that queue item is naturally removed / replaced / re-randomized. The old `` `${artist}::${song}` `` fallback identity scheme is gone.

### Queue constraints

- Maximum length: configurable per project (`projectConfig.shortsQueue.length`, default `20`)
- Duplicate spacing: the same entry cannot appear within the last `projectConfig.shortsQueue.duplicateSpacing` positions (default `2`)
- Exclusion: entries with `excludeFromRandomizer: true` are never added to the queue
- `buildQueue` makes up to `queueLength × 50` attempts before giving up (handles small libraries)
...
```json
{
  "illegalMindCovers": {
    "queue": [
      "3f7c1e94-2a6b-4d18-9c5e-8b1a2f0d6e77",
...
(A legacy queue may still contain `"blink-182-all the small things"`-style strings; those resolve via `matchKey` until naturally replaced.)
...
erDiagram
    ...
    SHORTS_QUEUE }o--o{ SAVED_ENTRIES : "queue = saved-entry UUID refs"
```
...
- `shortsQueues[projectId].queue[]` → saved-entry **id references** (UUIDs; legacy `matchKey` strings resolved as a fallback), resolved to live entries at render — not snapshot copies

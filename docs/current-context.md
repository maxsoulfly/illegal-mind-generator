...
- **Project Settings → Thumbnail Templates built** — new `src/components/projectSettings/ProjectSettingsThumbnails.jsx`, wired into `ProjectSettingsContent.jsx`. 4 cards (`tag-library tag-library--3col` grid, `TemplateGroupCard`): Words, Fallbacks, Generic Tag Templates, and a combined Patterns card (Long + Shorts as two `HookTemplateEditor noWrapper` instances in one card). Edits `thumbnail.words`/`.fallbacks`/`.genericTagTemplates`/`.patterns.{long,shorts}` — the exact keys `generateThumbnails.js` reads; `buildResolvedProjectConfig.js` already merged these correctly, no engine changes needed. Added `placeholders` prop to `HookTemplateEditor`/`TemplateGroupCard` (default `HOOK_PLACEHOLDERS`, backward compatible) so Thumbnail fields can pass `[]` (words/fallbacks — no substitution at all in the engine) or the new `THUMBNAIL_TAG_PLACEHOLDER` (`{tag}` only, for Generic Tag Templates) instead of the misleading full hook-placeholder list. No click-to-source nav added on the "Thumb:" output line — `generateThumbnails.js` doesn't track sourcing; noted as a possible follow-up.

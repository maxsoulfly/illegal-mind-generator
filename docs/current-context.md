...
Tag-category placeholders, title output polish, and random hook-block line selection all shipped (2026-07-04). Picking next feature.
...
The app is functionally complete for its core feature set. This session fixed a shorts-queue staleness bug (id references instead of frozen snapshots), added tag-category-based placeholders (`{tags.*}`) across all three template-filling engines, polished title output (removed the "Title:" label, consistent capitalization), and fixed hook blocks so any block's configured "Lines" count is actually honored (previously only two hardcoded blocks worked correctly). Next priority is new features: generic/no-tags title mode, queue-hidden indicators, or Todo status badges in the library.

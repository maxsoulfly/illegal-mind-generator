...
//   generation   (kind: 'workspace') — a persistent leaf switcher
//                                      (Titles / Short Hooks / Thumbnails /
//                                      Hashtags) + the editor below it.
//   descriptions (kind: 'workspace') — a persistent category switcher
//                                      (Layouts / Blocks / Variables, see
//                                      DESCRIPTION_GROUPS) + a leaf switcher
//                                      for the active category + the editor.
//                                      The 8 leaf ids are unchanged
//                                      (long/shorts + the 5 BLOCK_TYPE_SUBTABS
//                                      ids + links) so every blocksTarget /
//                                      openBlocksEditor deep-link lands on its
//                                      editor directly; the category is
//                                      always derived from the leaf via
//                                      groupOfDescriptionLeaf, never stored.
...
//
// `kind` is only read to tell a leaf-nav section ('workspace') from a single
// page ('page').
//
...
export const CONTENT_SETUP_SECTIONS = [
  {
    id: 'generation',
    label: 'Generation',
    // Persistent leaf switcher — a SubTabNav of these 4 + the editor below.
    kind: 'workspace',
    leaves: [
  ...
  },
  {
    id: 'descriptions',
    label: 'Descriptions',
    // Persistent category switcher + leaf switcher. The 8 leaves stay a flat
    // array here — dispatchableLeaves / sectionOfLeaf / the blocksTarget
    // deep-link path all read it. DESCRIPTION_GROUPS below is the category
    // model the top row is built from; the active category is always derived
    // from the current leaf (groupOfDescriptionLeaf), never stored. Leaf ids
    // unchanged: `long`/`shorts` + the BLOCK_TYPE_SUBTABS ids
    // (lists/text/hooks/groups/placeholders) + `links`, so blocksTarget.subTab
    // and every openBlocksEditor deep-link land on their editor directly.
    kind: 'workspace',
    leaves: [
      { id: 'long', label: 'Long' },
...
// ids are the same ones in CONTENT_SETUP_SECTIONS.descriptions; a leaf's
// category is `groupOfDescriptionLeaf(leafId)`. Conceptual flow:
// Variables -> Blocks -> Layouts -> Descriptions.
...
const LEGACY_SECTION_MAP = {
  ...
  descriptions:   { section: 'descriptions', leaf: 'long' }, // bare id / stale value -> default leaf
...
// Legacy map is checked before the passthrough. A bare 'descriptions' (a
// stale persisted value from the removed overview, or an idempotent section
// id) resolves to { section: 'descriptions', leaf: 'long' } — the default
// leaf. The Generator "DESCRIPTIONS" panel deep-links via 'long' directly.

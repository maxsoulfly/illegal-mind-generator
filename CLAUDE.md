...
- `CollapsiblePanel` (`src/components/ui/CollapsiblePanel.jsx`) — the `panel`/`panel-collapsed` + `panel-header` + `ToggleButton` shell shared by every Generator output panel (`HashtagsPanel`, `YouTubeTagsPanel`, `TitlesPanel`, `ShortHooksPanel`, `DescriptionsPanel`). Takes `label, visible, onToggle, onNavigate?, headerExtra?, children`. `onNavigate` swaps the plain `<h2 className="panel-title">` for a clickable `panel-title--nav` button (Titles/Descriptions link back to Project Settings). `headerExtra` renders between the title and the `ToggleButton` (Titles' "Hooks: ON/OFF" toggle). Plain `.panel` (no header/toggle — `ProjectSettingsPage`, `GeneratorPage`, `GeneratorResultsPanel`, `SavedLibrary`) is a separate, still-valid bare-wrapper usage — not every `.panel` needs to become a `CollapsiblePanel`.
